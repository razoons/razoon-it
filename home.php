  ...
  <?php $rank++;$previous_team_progress=$list_teams['production_progress']; } ?>
	...
	if ($_SESSION['current_turn']<>1){?>
		...
		<?php
			...
			$req_reports_deal=$bdd->query('SELECT COUNT(*) as total_hack, SUM(blocked) as total_blocked FROM actions WHERE action="deal" AND game_id='.$_SESSION['game_id'].' AND turn='.$previous_turn.' AND team_id='.$reports_actions['team_id'].'');
			$reports_deal=$req_reports_deal->fetch();
			if (is_null($reports_deal['total_blocked'])){$reports_deal['total_blocked']=0;}
			$req_reports_deal_2=$bdd->query('SELECT COUNT(*) as total_hack, SUM(blocked) as total_blocked FROM actions WHERE action="hack" AND game_id='.$_SESSION['game_id'].' AND turn='.$previous_turn.' AND target_team_id='.$reports_actions['team_id'].'');
			$reports_deal_2=$req_reports_deal_2->fetch();
			if (is_null($reports_deal_2['total_blocked'])){$reports_deal_2['total_blocked']=0;}
			...
			<?php }elseif ($reports_actions['action']=="deal"){ ?>
				<div class="result_team"><img src="./resources/deal.png"></div><span class="title_result">You were <?php echo $reports_deal['total_hack']+$reports_deal_2['total_hack'];?> user(s) hacking <b><?php echo $teams['team'][$reports_actions['team_id']];?></b> (<?php echo $reports_deal['total_blocked']+$reports_deal_2['total_blocked'];?> were blocked).<br/>
				<?php if($reports_actions['blocked']==0){?>
					<span class="good">Your hack was successful.</span>
				<?php }else{?>
					<span class="bad">Your hack has failed.</span>
